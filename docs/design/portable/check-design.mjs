// Guardián del sistema de diseño v1 de Electrificarte, versión portable (dashboard, vendedores).
//   node scripts/check-design.mjs           → agregalo como "design:check" en package.json
// Copia de scripts/qa/check-design.mjs de electrificarteweb. En proyectos con shadcn/ui
// (detecta components.json) no marca `primary` ni rounded-sm/md/lg/xl: ahí esos nombres se
// remapean a tokens del sistema en shadcn-theme.css.
//
...
// README del paquete de diseño (docs/design/portable en electrificarteweb)
//
...
const SHADCN = existsSync("components.json");

const RULES = [
  // — Paleta anterior ———————————————————————————————————————————————————————
  { re: /#00E5E5|#006A61/i,                          msg: "color de la paleta anterior (cyan)", fix: "usá los tokens semánticos: bg-accent / text-link / bg-accent-soft" },
  { re: SHADCN ? /\b(?:text|bg|border|ring|from|via|to|fill|stroke|divide)-primary-deep\b/ : /\b(?:text|bg|border|ring|from|via|to|fill|stroke|divide)-primary(?:-deep)?\b/, msg: "token `primary` del sistema anterior", fix: "bg-accent, text-link, border-line… (capa semántica)" },
  ...
  // — Forma —————————————————————————————————————————————————————————————————
  { re: SHADCN ? /\brounded-(?:2xl|3xl)\b/ : /\brounded-(?:sm|md|lg|xl|2xl|3xl)\b/,        msg: "radio fuera de la escala", fix: "rounded-chip (4) · rounded-control (8) · rounded-card (12)" },
...
const cssFiles = [...globSync("{src/,}app/**/*.css")];
...
console.log(`\n\x1b[31m${findings.length} infracción(es).\x1b[0m Reglas y equivalencias: README del paquete de diseño (docs/design/portable en electrificarteweb)`);
