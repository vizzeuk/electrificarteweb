...
import { formatCLP, carStats, cleanSeparators, cn, normalizeElectricLabel, sentenceCase, DEFAULT_HOT_DEAL_LABEL } from "@/lib/utils";
...
export interface ElectricoMeta {
  ...
  icon: string;
  /** Ya no se usa: el sistema v1 no tiene un color por tipo eléctrico. */
  color: string;
  tagline: string;
  description: string;
...
// ─── Copy por tecnología ─────────────────────────────────────────────────────
// Cómo se nombra cada tipo a mitad de frase ("marcas con un híbrido enchufable",
// "Todos los híbridos enchufables"). Si llega un tag nuevo se arma desde la etiqueta.
const TYPE_WORDS: Record<string, { one: string; many: string }> = {
  EV: { one: "un eléctrico puro", many: "autos 100% eléctricos" },
  BEV: { one: "un eléctrico puro", many: "autos 100% eléctricos" },
  PHEV: { one: "un híbrido enchufable", many: "híbridos enchufables" },
  HEV: { one: "un híbrido clásico", many: "híbridos clásicos" },
  MHEV: { one: "un micro híbrido", many: "micro híbridos" },
  EREV: { one: "un auto de autonomía extendida", many: "autos de autonomía extendida" },
  REEV: { one: "un auto de autonomía extendida", many: "autos de autonomía extendida" },
};
...
export default function ElectricoPageContent({ slug, meta, cars, otherTypes, adCar, plpBanners = [], hotDealUrgencyLabel }: ElectricoPageContentProps) {
  ...
  const tagLabel = electricTypeLabel(meta.tag) ?? meta.tag;
  const words = TYPE_WORDS[meta.tag.toUpperCase()] ?? { one: `un ${inlineLabel(meta.label)}`, many: `modelos ${tagLabel}` };
  const n = cars.length;
  ...
  const discounted = cars.filter((c) => c.discountPrice > 0 && c.discountPrice < c.basePrice).length;
  const kpis = [
    n > 0 && { num: String(n), label: n === 1 ? "modelo en el catálogo" : `${words.many} en el catálogo` },
    ...
    brandCount > 0 && { num: String(brandCount), label: `${brandCount === 1 ? "marca" : "marcas"} con ${words.one}` },
    discounted > 0 && { num: String(discounted), label: discounted === 1 ? "modelo con descuento" : "modelos con descuento" },
  ...
  return (
    <div className="page">
      ...
      <section className="page-head">
        <div className="wrap">
          <nav className="crumbs" aria-label="Migas de pan">
            ...
            <span>Tipo de electrificado</span>
            <span aria-hidden="true">/</span>
          ...
          <div className={cn("page-head__grid", adCar ? "page-head__grid--feature" : "grid-cols-1")}>
            <div>
              <div className="head-chips">
                <span className="chip">{tagLabel}</span>
              </div>
              <h1 className="t-h1">{meta.label}</h1>
              {meta.tagline && <p className="t-lead">{meta.tagline}</p>}
              {meta.description && <p className="page-head__desc">{meta.description}</p>}
      ...
      {/* ─── Cómo funciona (howItWorks de Sanity) ────────────────────── */}
      {meta.howItWorks.length > 0 && (
        <section className="section section--subtle" aria-labelledby="how-t">
          <div className="wrap">
            <div className="section-head">
              <div className="section-head__text">
                <h2 className="t-h2" id="how-t">Cómo funciona {words.one}</h2>
              </div>
            </div>
            <div className="how4">
              {meta.howItWorks.map((h) => (
                <div key={h.title}>
          ...
          <div className="wrap">
            <div className="deal__head">
              <div className="deal__eyebrow">
                <h2 className="chip chip--soft" id="deals-t">Ofertas destacadas en {words.many}</h2>
      ...
      <section className="section" id={`catalogo-${slug}`} aria-labelledby="cat-t">
        <div className="wrap">
          ...
          <div className="section-head mb-6">
            <div className="section-head__text">
              <h2 className="t-h2" id="cat-t">Todos los {words.many}</h2>
            </div>
          </div>

          {n === 0 ? (
            <p className="empty">Estamos cargando el catálogo {tagLabel}. Vuelve pronto.</p>
          ) : (
            <>
              <PlpFilters
      ...
      {/* ─── Ventajas, a tener en cuenta e ideal para ────────────────── */}
      {(meta.pros.length > 0 || meta.cons.length > 0 || meta.idealFor) && (
        <section className="section section--subtle" aria-labelledby="pc-t">
          <div className="wrap">
            <div className="section-head">
              <div className="section-head__text">
                <h2 className="t-h2" id="pc-t">¿Es {words.one} para ti?</h2>
              </div>
            </div>
            <div className="proscons">
              {meta.pros.length > 0 && (
                <div>
                  <h3 className="t-h3">Ventajas</h3>
                  <ul>
                    {meta.pros.map((p) => (
                      <li key={p}>
              ...
              {meta.cons.length > 0 && (
                <div className="cons">
                  <h3 className="t-h3">A tener en cuenta</h3>
                  <ul>
                    {meta.cons.map((c) => (
                      <li key={c}>
                        <Icon name="remove" size="none" />
                        <span>{c}</span>
                      </li>
                    ))}
                  </ul>
              ...
              {meta.idealFor && (
                <div>
                  <h3 className="t-h3">Ideal para</h3>
                  <p>{meta.idealFor}</p>
                </div>
              )}
            </div>
        ...
        <div className="wrap">
          {others.length > 0 && (
            <>
              <div className="section-head">
                <div className="section-head__text">
                  <h2 className="t-h2" id="ot-t">Otros tipos de electrificado</h2>
                </div>
              </div>
              <div className="type-links">
                {others.map((t) => (
                  <Link key={t.slug} href={`/electrico/${t.slug}`} className="type-link">
                    {normalizeElectricLabel(t.tag, t.label) ?? t.label}
                    <span className="chip">{electricTypeLabel(t.tag) ?? t.tag}</span>
                  </Link>
                ))}
              </div>
            </>
