...
`).trim(),e}async function w(a,e,signal){const t=await fetch(`${a}/api/chat`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({messages:e}),signal});if(!t.ok){const i=await t.json().catch(()=>({}));throw new Error(i.error||"Error al conectar con el servidor.")}return(await t.json()).message??""}async function S(a,e,signal){const t=await fetch(`${a}/api/chat`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({mode:"recommend",...e}),signal});if(!t.ok){const i=await t.json().catch(()=>({}));throw new Error(i.error||"Error al conectar con el servidor.")}return(await t.json()).message??""}const UPSELL_THRESHOLD=3,UPSELL_URL="https://app.reveniu.com/checkout-custom-link/nd1Zh0zfeNfi1b1yJgH8XeI94hJqycjB",EV_COUNT_KEY="ev_chat_free_count";const h=20,p="ev_chat_history",f=`¡Hola! Soy Francisco de Electrificarte. ¿En qué puedo ayudarte hoy?
...
5. Contratar asesoría personalizada
[/MENU]`,u=[{key:"budget",question:"¿Cuál es tu presupuesto aproximado?",options:[{label:"Hasta 15 millones CLP",value:"hasta-15",display:"hasta 15 millones CLP"},{label:"15 a 30 millones CLP",value:"15-30",display:"15 a 30 millones CLP"},{label:"30 a 50 millones CLP",value:"30-50",display:"30 a 50 millones CLP"},{label:"Más de 50 millones CLP",value:"mas-50",display:"más de 50 millones CLP"}]},{key:"vehicleType",question:"¿Qué tipo de carrocería prefieres?",options:[{label:"SUV o Crossover",value:"suv",display:"SUV o Crossover"},{label:"Sedán",value:"sedan",display:"Sedán"},{label:"Hatchback o City car",value:"hatchback",display:"Hatchback o City car"},{label:"Pickup",value:"pickup",display:"Pickup"},{label:"Sin preferencia",value:"any",display:"cualquier carrocería"}]},{key:"electricType",question:"¿Prefieres un auto 100% eléctrico o también consideras híbridos?",options:[{label:"100% eléctrico",value:"electric",display:"100% eléctrico"},{label:"Híbrido o enchufable",value:"hybrid",display:"híbrido o enchufable"},{label:"Me da igual",value:"any",display:"cualquier tipo"}]}];class E extends HTMLElement{constructor(){super(),this._shadow=this.attachShadow({mode:"open"}),this._history=[],this._isOpen=!1,this._loading=!1,this._savingsState=null,this._savingsKm=null,this._findStep=-1,this._findCriteria={},this._abortCtrl=null,this._freeCount=0}disconnectedCallback(){clearTimeout(this._nudgeTimer),this._abortCtrl?.abort()}connectedCallback(){this._apiUrl=this.dataset.apiUrl||"",this._botName=this.dataset.botName||"Francisco Electrificarte",this._primaryColor=this.dataset.primaryColor||"#00E5E5";const e=document.createElement("style");e.textContent=v(this._primaryColor),this._shadow.appendChild(e);const t=document.createElement("div");t.innerHTML=y(this._botName),this._shadow.appendChild(t),this._launcher=this._shadow.getElementById("launcher"),this._panel=this._shadow.getElementById("panel"),this._messagesEl=this._shadow.getElementById("messages"),this._input=this._shadow.getElementById("input"),this._btnSend=this._shadow.getElementById("btn-send"),this._btnClear=this._shadow.getElementById("btn-clear"),this._launcher.addEventListener("click",()=>this._togglePanel()),this._btnSend.addEventListener("click",()=>this._handleSend()),this._btnClear.addEventListener("click",()=>this._clearChat()),this._input.addEventListener("keydown",s=>{s.key==="Enter"&&!s.shiftKey&&(s.preventDefault(),this._handleSend())}),this._input.addEventListener("input",()=>{this._btnSend.disabled=this._input.value.trim()===""||this._loading,this._input.style.height="auto",this._input.style.height=Math.min(this._input.scrollHeight,80)+"px"}),this._restoreSession(),this._initNudge()}_initNudge(){this._nudgeEl=this._shadow.getElementById("nudge");const e=this._shadow.getElementById("nudge-close");if(!this._nudgeEl||sessionStorage.getItem("ev_nudge_seen"))return;e.addEventListener("click",t=>{t.stopPropagation(),this._hideNudge(!0)}),this._nudgeEl.addEventListener("click",()=>{this._hideNudge(!0),this._togglePanel()}),this._nudgeTimer=setTimeout(()=>{this._nudgeEl&&!this._isOpen&&(this._nudgeEl.classList.remove("hidden"),requestAnimationFrame(()=>this._nudgeEl.classList.add("visible")))},30*1e3)}_hideNudge(e=!1){if(!this._nudgeEl)return;this._nudgeEl.classList.remove("visible"),setTimeout(()=>this._nudgeEl&&this._nudgeEl.classList.add("hidden"),300),e&&sessionStorage.setItem("ev_nudge_seen","1"),clearTimeout(this._nudgeTimer)}_saveSession(){try{sessionStorage.setItem(p,JSON.stringify(this._history)),sessionStorage.setItem(EV_COUNT_KEY,String(this._freeCount))}catch{}}_restoreSession(){try{const e=sessionStorage.getItem(p);if(e&&(this._history=JSON.parse(e),this._history.length>0)){this._freeCount=parseInt(sessionStorage.getItem(EV_COUNT_KEY)||"0",10)||0;for(const t of this._history)c(this._messagesEl,t.role==="user"?"user":"bot",t.content,(s,i)=>this._handleMenuSelect(s,i));return}}catch{sessionStorage.removeItem(p),sessionStorage.removeItem(EV_COUNT_KEY)}this._showBotMessage(f)}_togglePanel(){this._isOpen=!this._isOpen,this._launcher.classList.toggle("open",this._isOpen),this._panel.classList.toggle("visible",this._isOpen),this._panel.setAttribute("aria-hidden",String(!this._isOpen)),this._launcher.setAttribute("aria-expanded",String(this._isOpen)),this._isOpen&&(requestAnimationFrame(()=>this._input.focus()),this._hideNudge(!0))}_clearChat(){this._abortCtrl?.abort(),this._abortCtrl=null,this._loading=!1,this._history=[],this._freeCount=0,this._savingsState=null,this._savingsKm=null,this._findStep=-1,this._findCriteria={},this._messagesEl.innerHTML="",sessionStorage.removeItem(p),sessionStorage.removeItem(EV_COUNT_KEY),this._btnSend.disabled=this._input.value.trim()==="",this._showBotMessage(f)}_showUpsell(){const e=document.createElement("div");e.className="upsell-card",e.innerHTML=`<div class="upsell-label">Asesoría Personalizada</div><div class="upsell-title">¿Quieres atención 100% personalizada?</div><div class="upsell-desc">Resuelve todas tus dudas directamente por WhatsApp con un experto Electrificarte — solo $4.990 CLP.</div><ul class="upsell-benefits"><li>✅ +50 personas asesoradas</li><li>✅ Recomendación ajustada a tu realidad y presupuesto</li><li>✅ Comparación de +120 autos eléctricos e híbridos</li><li>✅ Atención directa y personalizada por WhatsApp</li></ul><a class="upsell-btn" href="${UPSELL_URL}">Contratar asesoría &middot; $4.990 →</a><div class="upsell-post">Luego del pago te contactaremos directamente por WhatsApp.</div><button class="upsell-no-btn">No me interesa por ahora</button>`,this._messagesEl.appendChild(e),this._messagesEl.scrollTop=this._messagesEl.scrollHeight;const t=e.querySelector(".upsell-no-btn");t&&t.addEventListener("click",()=>{e.remove(),this._showRetentionMessage()})}_showRetentionMessage(){this._showBotMessage(`¿Estás seguro? 🤔 Con la asesoría podrías tener acceso a:\n\n- ✅ +50 personas ya asesoradas exitosamente\n- ✅ Recomendación 100% ajustada a tu realidad y presupuesto\n- ✅ Comparación entre más de 120 autos eléctricos e híbridos\n- ✅ Atención directa y personalizada por WhatsApp\n\nTodo eso por solo **$4.990 CLP**.\n\n[MENU]\n1. Contratar asesoría → ${UPSELL_URL}\n2. Continuar sin asesoría\n[/MENU]`)}_showBotMessage(e){c(this._messagesEl,"bot",e,(t,s)=>this._handleMenuSelect(t,s))}_handleMenuSelect(e,t){this._loading||this._submitUserMessage(`${e}. ${t}`)}async _handleSend(){const e=this._input.value.trim();!e||this._loading||(this._input.value="",this._input.style.height="auto",this._btnSend.disabled=!0,this._submitUserMessage(e))}async _submitUserMessage(e){if(e.includes("Empezar de nuevo")||e.includes("Volver al inicio")){c(this._messagesEl,"user",e,()=>{}),this._clearChat();return}if(this._isSavingsFlow(e)){this._handleSavingsFlow(e);return}if(this._isContactFlow(e)){this._handleContactFlow();return}if(this._isFindFlow(e)){this._handleFindFlow(e);return}if(/asesor[ií]a/i.test(e)&&!e.startsWith("Contratar")&&!e.startsWith("No me interesa")){c(this._messagesEl,"user",e,()=>{});this._history.push({role:"user",content:e});this._saveSession();this._showBotMessage("¡Claro! Para eso tenemos nuestra asesoría directa con un experto de Electrificarte 🎯");this._showUpsell();return}c(this._messagesEl,"user",e,()=>{}),this._history.push({role:"user",content:e}),this._history.length>h&&(this._history=this._history.slice(-h)),this._abortCtrl?.abort(),this._abortCtrl=new AbortController(),this._loading=!0,this._btnSend.disabled=!0;const t=b(this._messagesEl);try{const s=await w(this._apiUrl,this._history,this._abortCtrl.signal);t(),this._history.push({role:"assistant",content:s}),this._history.length>h&&(this._history=this._history.slice(-h)),this._freeCount++,this._saveSession(),this._showBotMessage(s),this._freeCount===UPSELL_THRESHOLD&&this._showUpsell()}catch(err){if(err?.name==="AbortError"){t();return}t(),this._showBotMessage(`Lo siento, hubo un problema al conectar. Por favor intenta nuevamente.
...
[MENU]
${t}
[/MENU]`)}async _callRecommendation(){const{budget:e,vehicleType:t,electricType:s}=this._findCriteria,i=`Busco ${t.display}, ${s.display}, presupuesto ${e.display}`;this._abortCtrl?.abort(),this._abortCtrl=new AbortController(),this._loading=!0,this._btnSend.disabled=!0;const n=b(this._messagesEl);try{const d=await S(this._apiUrl,{summary:i,budget:e.value,vehicleType:t.value,electricType:s.value},this._abortCtrl.signal);n(),this._history.push({role:"user",content:i}),this._history.push({role:"assistant",content:d}),this._history.length>h&&(this._history=this._history.slice(-h)),this._freeCount++,this._saveSession(),this._showBotMessage(d),this._freeCount===UPSELL_THRESHOLD&&this._showUpsell()}catch(err){if(err?.name==="AbortError"){n();return}n(),this._showBotMessage(`Lo siento, hubo un problema al buscar opciones. Por favor intenta nuevamente.

[MENU]
1. Volver al inicio
[/MENU]`)}finally{this._loading=!1,this._btnSend.disabled=this._input.value.trim()===""}}_isSavingsFlow(e){return this._savingsState!==null?!0:e==="3. ¿Cuánto ahorro versus bencina?"||e==="3"&&this._findStep<0}_handleSavingsFlow(e){if(this._savingsState===null){window.location.href="/calculadora";return}if(this._savingsState===null){c(this._messagesEl,"user",e,()=>{}),this._savingsState="ask_km",this._showBotMessage(`¿Cuántos kilómetros recorres al mes aproximadamente?
