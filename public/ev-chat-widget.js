...
`).trim(),e}async function w(a,e,signal){const t=await fetch(`${a}/api/chat`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({messages:e}),signal});if(!t.ok){const i=await t.json().catch(()=>({}));throw new Error(i.error||"Error al conectar con el servidor.")}return(await t.json()).message??""}async function S(a,e,signal){const t=await fetch(`${a}/api/chat`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({mode:"recommend",...e}),signal});if(!t.ok){const i=await t.json().catch(()=>({}));throw new Error(i.error||"Error al conectar con el servidor.")}return(await t.json()).message??""}const UPSELL_THRESHOLD=7,UPSELL_URL="https://pay.electrificarte.com/asesoria",EV_COUNT_KEY="ev_chat_free_count";const h=20,p="ev_chat_history",f=`¡Hola! Soy Francisco de Electrificarte. ¿En qué puedo ayudarte hoy?
