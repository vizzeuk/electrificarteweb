...
async function getOrCreateBrand(name: string): Promise<string> {
  ...
  const doc = await sanity.create({ _type: "brand", name, slug: { _type: "slug", current: slug } });
...
async function uploadImage(url: string): Promise<string | null> {
  try {
    ...
    console.log(`    ! error descargando foto ${url}: ${(err as Error).message}`);
    return null;
  }
}
...
async function runSourceSearch(systemPrompt: string, brand: string, model: string): Promise<SourceReport> {
  ...
  return {
    found: !!input.found && urls.length > 0,
    urls,
    note: input.note,
  };
}

async function findOfficialSources(brand: string, model: string): Promise<SourceReport> {
  console.log(`\n▶ Buscando ficha oficial de ${brand} ${model} en Chile...`);
  return runSourceSearch(OFFICIAL_SYSTEM_PROMPT, brand, model);
}

async function findAuthorizedDealerSources(brand: string, model: string): Promise<SourceReport> {
  console.log(`▶ Buscando concesionario autorizado de ${brand} en Chile (respaldo)...`);
  return runSourceSearch(DEALER_SYSTEM_PROMPT, brand, model);
}
...
async function scrapePage(url: string, browser: import("playwright").Browser): Promise<PageScrape | null> {
...
async function fetchPdfText(url: string): Promise<string | null> {
  try {
    ...
    console.log(`    ✗ ${url} (PDF) — ${(err as Error).message}`);
    return null;
  }
}
...
async function fetchOne(url: string, browser: import("playwright").Browser): Promise<FetchedPage | null> {
  const isPdf = /\.pdf($|\?)/i.test(new URL(url).pathname);
  if (isPdf) {
    const text = await fetchPdfText(url);
    if (!text || !isUsableScrape(text)) return null;
    console.log(`    ✓ ${url} (PDF) — ${text.length} chars`);
  ...
  if (!scraped || !isUsableScrape(scraped.text)) {
    if (scraped) console.log(`    ✗ ${url} — página bloqueada o sin contenido útil (${scraped.text.length} chars), se descarta`);
    return null;
  }
  console.log(
...
async function gatherSource(
  ...
  visited.add(url);
  const primary = await fetchOne(url, browser);
  ...
  for (const link of primary.specLinks) {
    ...
    visited.add(link);
    const extra = await fetchOne(link, browser);
    if (extra) {
...
async function gatherAll(
  ...
  for (const url of urls) {
    const gathered = await gatherSource(url, browser, visited);
    if (gathered) {
...
async function extractSpecs(brand: string, model: string, corpus: string): Promise<Record<string, unknown>> {
  console.log(`▶ Extrayendo specs desde el material recolectado (${corpus.length} caracteres)...`);
...
async function main() {
  ...
  if (existing) {
    console.log(`\n= Ya existe un auto con slug "${slug}" (${existing}) — no se crea nada.\n`);
    return;
  }

  let sources = await findOfficialSources(brand, model);
  if (sources.found && sources.urls.length > 0) {
    console.log(`  ✓ ${sources.urls.length} fuente(s):`);
    sources.urls.forEach((u) => console.log(`    - ${u}`));
  } else {
    console.log(`  No se encontró fuente oficial confiable.${sources.note ? ` Nota: ${sources.note}` : ""}`);
  }
  ...
  let { texts: chunks, images: allImages } = sources.urls.length
    ? await gatherAll(sources.urls, browser, visited)
  ...
  if (chunks.length === 0) {
    ...
    const dealerSources = await findAuthorizedDealerSources(brand, model);
    if (dealerSources.found && dealerSources.urls.length > 0) {
      console.log(`  ✓ ${dealerSources.urls.length} fuente(s) de concesionario:`);
      dealerSources.urls.forEach((u) => console.log(`    - ${u}`));
      const dealerResult = await gatherAll(dealerSources.urls, browser, visited);
      ...
      console.log(`  Nota (fallback): ${dealerSources.note}`);
    }
  }
  await browser.close();

  if (chunks.length === 0) {
    console.log("\n✗ Ninguna fuente (oficial ni concesionario) devolvió contenido útil. No se crea nada — revisar manualmente.\n");
    return;
  }
  ...
  const specs = await extractSpecs(brand, model, chunks.join("\n\n"));

  const brandId = await getOrCreateBrand(brand);
  const vehicleTypeId = await resolveVehicleType((specs.vehicleType as string) ?? null);
  const electricTypeId = await resolveElectricType((specs.electricType as string) ?? null);
  if (!vehicleTypeId) console.log(`  ! tipo de vehículo "${specs.vehicleType}" no encontrado — queda vacío`);
  ...
  if (!electricTypeId) {
    console.log(
      ...
      `${brand} ${model} en el material fuente (specs.electricType: "${specs.electricType ?? "—"}"). ` +
      `Electrificarte solo cataloga autos electrificados — no se crea la ficha. Revisar manualmente.\n`
    );
    return;
  }
  ...
  if (DRY_RUN) {
    ...
    if (uniqueImages.length < MIN_DESIRED_PHOTOS) {
      console.log(`  ⚠ Menos de ${MIN_DESIRED_PHOTOS} candidatas (1 portada + 6 galería mínimo deseado).`);
    }
  } else {
    ...
    for (const imgUrl of uniqueImages) {
      ...
      if (assetId) uploadedIds.push(assetId);
    }
    if (uploadedIds.length > 0) {
      mainImage = imageRef(uploadedIds[0]);
      // La galería incluye la portada primero, igual que el resto del catálogo.
      gallery = uploadedIds.map((id, i) => imageRef(id, `img${i}`));
    }
    if (gallery.length < MIN_DESIRED_PHOTOS) {
      console.log(`  ⚠ Solo ${gallery.length}/${MIN_DESIRED_PHOTOS} fotos subidas — completar manualmente en Studio.`);
    } else {
      console.log(`  ✓ ${gallery.length} fotos subidas (incluye portada).`);
    }
  }
  ...
  console.log(`\n✓ Creado: ${result._id}`);
  console.log(`  Revisar en Studio: ${studioBase}/structure/car;${result._id}`);
  console.log(`  (hidden:true — no aparece en el sitio hasta que lo publiques)\n`);
}
